import React, { useEffect, useState } from 'react';

import './App.css';
import { getBreeds } from './functions/get-breeds/get-breeds';

import { SpeciesButton } from './components/species-button';
import { BreedSelector } from './components/breed-selector';
import { Question } from './components/question';
import { SpeciesSelectedStatus } from './types/enum';
import { DownButton } from './components/down-button/down-button';
import { CustomHeader } from './components/custom-header';


function App() {

  //Had a hard time getting the cat image from Figma. Recalled seeing these in the quote funnel.
  const dogPicUrl="https://images.unsplash.com/photo-1600804340584-c7db2eacf0bf?ixlib=rb-1.2.1&q=80&fm=jpg&crop=entropy&cs=tinysrgb&w=1080&fit=maxhttps://www.google.com"
  const catPicUrl="data:image/jpeg;base64,"
  
  const [selectedSpecies, setSelectedSpecies] = useState(SpeciesSelectedStatus.Unselected);
  const [selectedBreed, setSelectedBreed] = useState("");
  const [catBreedList, setCatBreedList] = useState([""]);
  const [dogBreedList, setDogBreedList] = useState([]);

  //called at the rendering and when the user selects a species.
  useEffect(() => {
    //Handles locking the users screen until they select a species, then smoothly autoscrolls
    // to the search bar once they do.
    if(selectedSpecies===SpeciesSelectedStatus.Unselected){
      document.body.style.overflow = "hidden";
    }
    else{
      document.body.style.overflow = "visible";
      window.scrollTo({
        top: document.body.scrollHeight,
        left: 0,
        behavior: 'smooth'
      });
    }
    const fetchMyAPI = async () => {
      let allBreeds = await getBreeds();
      setDogBreedList(allBreeds[0] as [])
      setCatBreedList(allBreeds[1] as [])
    }

    fetchMyAPI()

  }, [selectedSpecies]);


  return (
   
    <div className="MainPet">
      <CustomHeader></CustomHeader>
      <Question passedText='Your pet is a?'></Question>
      <SpeciesButton speciesText='Dog' setSpecies={setSelectedSpecies}></SpeciesButton>
      <br></br>
      <SpeciesButton speciesText='Cat' setSpecies={setSelectedSpecies}></SpeciesButton>
      <br></br>
      <DownButton></DownButton>
      <BreedSelector allBreeds={selectedSpecies == SpeciesSelectedStatus.Cat ? catBreedList : dogBreedList} imageUrl={selectedSpecies == SpeciesSelectedStatus.Cat ? catPicUrl : dogPicUrl}></BreedSelector>
    </div>
  );
}

export default App;
